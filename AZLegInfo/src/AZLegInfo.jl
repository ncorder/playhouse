...
function download_voting_histories(url::AbstractString)
    """
    ...
    SharePoint/OneDrive links must be shared with "Anyone with the link";
    links limited to people in an organization need a login and fail with
    403. A local zip can be read with a file:// URL.

    Expects:
        url is the address of the .zip file
    ...
    Citations:
      Code generated by Claude Code, Anthropic, September 22, 2026, https://claude.ai/code/session_01SQ79sYVPmqwp7yFkriCVsL.
    """
    # A SharePoint sharing link opens a viewer page unless download=1.
    if occursin(r"^https://[^/]*\.sharepoint\.com/", url) && !occursin("download=1", url)
        url = string(url, occursin('?', url) ? "&" : "?", "download=1")
    end
    ...
    mktemp() do zip_path, io
        ...
        try
            ...
            if err isa Downloads.RequestError && err.response.status in (401, 403)
                error("Access denied ($(err.response.status)) downloading $url. " *
                      "If this is a SharePoint/OneDrive link, share it with \"Anyone with the link\".")
            end
            rethrow()
        end
